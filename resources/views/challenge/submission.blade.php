...
<div class="flex flex-1 w-full h-full flex-col pt-[50px] px-[100px] gap-[50px] box-border bg-[var(--color-cream-500)]">
    <a  href="{{ route('challenge-detail', ['id' => $challenge->id]) }}" class="text-[var(--color-green-700)] font-semibold text-[20px]">< Previous</a>
    ...
    <div class="flex justify-center w-full">
        <form enctype="multipart/form-data" class="w-[1111px] h-[700px] box-border px-8 py-4 gap-5 flex-col flex border-2 border-[var(--color-green-700)] mb-10" method="POST" action="{{ route("submit-challenge") }}">
        @csrf
        <input type="hidden" name="id" value="{{$challenge->id}}" />
        <div class="flex flex-col gap-1">
            <p class="text-green-700 font-semibold text-[30px]">Username</p>
            <input type="text" placeholder="{{Auth::user()->fullname}}" disabled class="w-full bg-white outline-none p-3 rounded-md border-2 border-[var(--color-green-700)] h-[57px]"/>
        </div>
        <div class="flex flex-col gap-1">
            <p class="text-green-700 font-semibold text-[30px]">Instagram Account</p>
            <input type="text" placeholder="Zerobon" class="w-full bg-white outline-none p-3 rounded-md border-2 border-[var(--color-green-700)] h-[57px]"/>
        </div>
        <div class="flex flex-col gap-1">
            <p class="text-green-700 font-semibold text-[30px]">Challenge</p>
            <input type="text" placeholder="{{$challenge->name}}" class="w-full bg-white outline-none p-3 rounded-md border-2 border-[var(--color-green-700)] h-[57px]"/>
        </div>
        <div class="flex flex-col gap-1">
            <p class="text-green-700 font-semibold text-[30px]">Upload File</p>
            ...
            <div class="w-full flex">
                ...
                <button class="bg-green-700 text-[var(--color-white)] h-[57px] w-[177px] font-bold text-[25px] rounded-md hover:cursor-pointer" id="choose_file_button">Choose</button>
            </div>
        </div>
        <div class="flex w-full justify-center">
            <input type="submit" value="Submit Challenge" class="w-[356px] h-[69px] text-white font-semibold text-[24px] bg-[var(--color-green-700)] rounded-[15px] mt-10"/>
        </div>
    </form>
    </div>
</div>
